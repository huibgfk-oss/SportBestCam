...
public class Utils {
    ...
    /**
     * Tries to parse the recording timestamp from current SportBestCam names and
     * legacy FadCam names. Supported examples include:
     *
     * SportBestCam_20260827_104100.mp4
     * SportBestCam_Handball_Team-A_vs_Team-B_20260827_104100.mp4
     * SportBestCam_Handball_Team-A_vs_Team-B_20260827_104100_part2.mp4
     * FadCam_20260827_104100.mp4
     *
     * The final yyyyMMdd_HHmmss token is used deliberately so descriptive match
     * titles and team names can safely contain other digits.
     *
    ...
    public static long parseTimestampFromFilename(String filename) {
        ...
        String currentPrefix = Constants.RECORDING_DIRECTORY + "_";
        String legacyPrefix = Constants.LEGACY_RECORDING_DIRECTORY + "_";
        String extension = "." + Constants.RECORDING_FILE_EXTENSION;

        if (!(filename.startsWith(currentPrefix) || filename.startsWith(legacyPrefix))
        ...
        try {
            ...
            Matcher matcher = Pattern.compile("(\\d{8}_\\d{6})(?:_part\\d+)?$").matcher(stem);
