...
public class DualCameraRecordingService extends Service {
    ...
    private void handleChangeSurface(@NonNull Intent intent) {
        ...
        if (recordingPipeline != null) {
            ...
            String previewOrientation = intent.getStringExtra("VIDEO_ORIENTATION");
            if (deviceOrientation != -1 || previewOrientation != null) {
                recordingPipeline.updatePreviewOrientation(previewOrientation, deviceOrientation);
            }
    ...
    private int getCurrentDisplayRotationForRecording() {
        try {
            ...
            FLog.w(TAG, "Unable to read display rotation for dual recording lock", e);
        }
    ...
    private void startDualRecording() {
        ...
        try {
            ...
            // Lock encoded geometry to the physical rotation at START.
            // Later rotations are preview-only and cannot mutate the current MP4.
            recordingPipeline.setRecordingDeviceOrientation(getCurrentDisplayRotationForRecording());
    ...
    private File createOutputFile() {
        ...
        String filename = com.fadcam.handball.SportRecordingProfile.buildFilename(
                this, "DualCam_", timestamp, "", Constants.RECORDING_FILE_EXTENSION);
