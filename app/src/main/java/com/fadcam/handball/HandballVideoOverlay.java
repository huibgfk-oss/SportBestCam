...
public final class HandballVideoOverlay {
    ...
    public static String append(Context context, String baseText) {
        ...
        if (!SportRecordingProfile.isMatchRecording(p)) return baseText == null ? "" : baseText;
        if (!p.getBoolean("video_overlay_enabled", true)) return baseText == null ? "" : baseText;

        String home = clean(SportRecordingProfile.teamA(context, p),
                context.getString(com.fadcam.R.string.sbc_team_a));
        String away = clean(SportRecordingProfile.teamB(context, p),
        ...
        String homeMarker = TeamColorMarker.marker(p.getInt("team_a_color", defaultA));
        String awayMarker = TeamColorMarker.marker(p.getInt("team_b_color", defaultB));
        ...
        if ("corner".equals(style)) {
            overlay = matchType + "\n"
                    + homeWithColor + "  " + hs + "\n"
                    + awayWithColor + "  " + as + "\n"
                    + String.format(Locale.ROOT, "%s  %02d:%02d",
                    period, sec / 60L, sec % 60L);
        } else if ("minimal".equals(style)) {
            overlay = String.format(Locale.ROOT,
                    "%s %d-%d %s · %02d:%02d · %s · %s",
                    homeWithColor, hs, as, awayWithColor,
                    sec / 60L, sec % 60L, period, matchType);
        } else {
            String scoreLine = String.format(Locale.ROOT,
                    "%s %d : %d %s   %02d:%02d   %s",
                    homeWithColor, hs, as, awayWithColor,
                    sec / 60L, sec % 60L, period);
            overlay = matchType + "\n" + scoreLine;
        }

        String base = baseText == null ? "" : baseText.trim();
        return base.isEmpty() ? overlay : base + "\n" + overlay;
    }
