...
public class GLRecordingPipeline {
    ...
    // Mutable preview orientation; recording orientation above remains final.
    private String previewOrientationSetting;
    ...
    // Current display rotation used by live preview.
    private int deviceOrientation = android.view.Surface.ROTATION_0;
    // Rotation locked at recording START and used only by the encoder.
    private int recordingDeviceOrientation = android.view.Surface.ROTATION_0;
    private volatile boolean frontVideoMirrorEnabled;
    // SportBestCam: locked at REC start. When true, encoder removes the outer
    // rear-camera gesture zones; live preview remains uncropped.
    private volatile boolean rearGestureCropEnabled;
    ...
    public void setRearGestureCropEnabled(boolean enabled) {
        rearGestureCropEnabled = enabled;
        if (glRenderer != null) {
            glRenderer.setRearGestureCropEnabled(enabled);
        }
    }
    ...
    public void prepareSurfaces() {
        try {
            ...
            if (glRenderer == null) {
                ...
                glRenderer.setPreviewOrientationSetting(previewOrientationSetting);
                glRenderer.setRecordingDeviceOrientation(recordingDeviceOrientation);
                ...
                glRenderer.setRearGestureCropEnabled(rearGestureCropEnabled);
    ...
    /** Locks the physical display rotation used by the encoder for this session. */
    public void setRecordingDeviceOrientation(int recordingDeviceOrientation) {
        this.recordingDeviceOrientation = recordingDeviceOrientation;
        // Start preview at the same rotation; later surface/config changes may update
        // deviceOrientation without touching the recording lock.
        this.deviceOrientation = recordingDeviceOrientation;
        if (glRenderer != null) {
            glRenderer.setRecordingDeviceOrientation(recordingDeviceOrientation);
            glRenderer.setDeviceOrientation(recordingDeviceOrientation);
        }
    }

    /** SportBestCam: rotate/reframe ONLY the live preview without tearing down the camera.
     * Current MP4 dimensions/orientation stay fixed; the next recording uses the
     * persisted orientation selected before START. */
    public void updatePreviewOrientation(@Nullable String videoOrientation, int deviceOrientation) {
        if (videoOrientation != null) {
            this.previewOrientationSetting = videoOrientation;
            if (glRenderer != null) {
                glRenderer.setPreviewOrientationSetting(videoOrientation);
            }
        }
