...
public class GLWatermarkRenderer {
    ...
    // SportBestCam 0002.7.0:
    // - deviceOrientation follows the CURRENT display rotation for live preview.
    // - recordingDeviceOrientation is locked when the recording session starts.
    // Keeping these separate prevents a running MP4 from rotating/stretching when
    // the phone is turned while recording.
    private int deviceOrientation = Surface.ROTATION_0;
    private int recordingDeviceOrientation = Surface.ROTATION_0;
    ...
    // Recording orientation is immutable for the lifetime of one encoder session.
    // Preview orientation may change when the device rotates.
    private String userOrientationSetting = "portrait";
    private String previewOrientationSetting = "portrait";

    // Sets the recording/session orientation. Called once when the renderer is created.
    public void setUserOrientationSetting(String orientation) {
        if (orientation == null) return;
        this.userOrientationSetting = orientation;
        this.previewOrientationSetting = orientation;
        updateMatrices();
    }

    // SportBestCam: updates ONLY the live preview orientation. The encoded MP4
    // keeps the orientation and dimensions it had at recording START.
    public void setPreviewOrientationSetting(String orientation) {
        if (orientation == null) return;
        this.previewOrientationSetting = orientation;
        updateMatrices();
    }

    public void setRecordingDeviceOrientation(int orientation) {
        this.recordingDeviceOrientation = normalizeSurfaceRotation(orientation);
        updateMatrices();
    }
    ...
    public GLWatermarkRenderer(Context context, Surface outputSurface, String orientation, int sensorOrientation,
        ...
        Matrix.translateM(rearGestureCropMatrix, 0,
                REAR_GESTURE_CROP_FRACTION, 0f, 0f);
        float rearGestureUsableWidth = 1f - (2f * REAR_GESTURE_CROP_FRACTION);
        Matrix.scaleM(rearGestureCropMatrix, 0, rearGestureUsableWidth, 1f, 1f);
    }
    ...
    private void renderToEncoderInternal(boolean allowStaleFrame) {
        synchronized (renderLock) {
            ...
            boolean isLandscape = isLandscapeOrientation(userOrientationSetting);
            ...
            // Rear hidden-zone fallback: crop only the encoded image horizontally.
            // Preview stays full-frame so the operator can use the outer gesture zones.
            // Never alter an active dual-camera/PiP composition.
            if (rearGestureCropEnabled && !pipEnabled) {
                Matrix.multiplyMM(rearGestureCroppedTexMatrix, 0,
                        encoderTexMatrix, 0, rearGestureCropMatrix, 0);
                encoderTexMatrix = rearGestureCroppedTexMatrix;
            }
            // Draw to encoder
            try {
                ...
                if (pipEnabled) {
                    updatePipTexture();
                    drawPipOverlay(pipMvpMatrix, false); // encoder uses locked recording orientation
                }
    ...
    public void renderToPreview() {
        ...
        synchronized (previewRenderLock) {
            ...
            // SportBestCam: optional zoom-fill/dim background for preview only.
            // Default ON preserves the historical FadCam behaviour. Turning it OFF
            // leaves clean black letterbox/pillarbox bars and never changes the
            // recorded/encoded video.
            boolean previewFillBackgroundEnabled = context
                    .getSharedPreferences("sportbestcam_ui", Context.MODE_PRIVATE)
                    .getBoolean("preview_fill_background", true);

            if (pvW[0] > 0 && pvH[0] > 0) {
                // Content dimensions = encoder output. If the phone is rotated
                // during recording, the encoder remains locked but the LIVE preview
                // must follow the new device orientation. Swap the preview content
                // aspect ratio in that case instead of stretching the encoder frame.
                int contentW = encoderWidth > 0 ? encoderWidth : videoWidth;
                int contentH = encoderHeight > 0 ? encoderHeight : videoHeight;

                boolean recordingLandscape = isLandscapeOrientation(userOrientationSetting);
                boolean previewLandscape = isLandscapeOrientation(previewOrientationSetting);
                if (recordingLandscape != previewLandscape) {
                    int tmp = contentW;
                    contentW = contentH;
                    contentH = tmp;
                }
                ...
                // ── Optional zoom-fill background ──
                // When enabled, draw the camera frame scaled to FILL/CROP the
                // entire preview and dim it behind the sharp aspect-ratio-correct
                // foreground. When disabled, keep the clean black bars.
                if (needsLetterbox && previewFillBackgroundEnabled) {
                    ...
                    // No background fill (or no letterbox) — render sharp frame only.
            ...
            // Preview follows the CURRENT phone orientation while encoder geometry
            // remains locked to userOrientationSetting.
            boolean isLandscape = isLandscapeOrientation(previewOrientationSetting);
            ...
            // Pass 1: optional zoom-fill background.
            if (needsLetterbox && previewFillBackgroundEnabled) {
            ...
            if (pipEnabled) {
                updatePipTexture();
                drawPipOverlay(pipMvpMatrix, true); // preview follows current device orientation
            }
    ...
    private void updateWatermarkTexture() {
        ...
        for (String line : lines) {
            ...
            if (line.contains("<FADCAM_ICON>")) {
                lineW += measureIconWidth("sportbestcam_mark", iconLineH) + (padding * 0.25f);
            }
     ...
     * {@code <FADCAM_ICON>} (SportBestCam icon) placeholders inline with the text.
    ...
    private void drawLineWithIcons(Canvas canvas, String line, float textY, int padding,
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (isFadrec == null) {
                ...
                // Icon segment
                String drawableName = isFadrec ? "fadrec" : "sportbestcam_mark";
    ...
    private void computePipGeometry() {
        ...
        int rotationDeg = getRequiredRotation(recordingDeviceOrientation);
     ...
     */
    private void drawPipOverlay(float[] mvpMatrix, boolean forPreview) {
        ...
        // Apply the orientation matching the target being rendered. Encoder keeps
        // the locked recording orientation; preview follows the current device.
        boolean isLandscape = isLandscapeOrientation(
                forPreview ? previewOrientationSetting : userOrientationSetting);
        ...
        int rotDeg = getRequiredRotation(recordingDeviceOrientation);
    ...
    public void setDeviceOrientation(int orientation) {
        int normalized = normalizeSurfaceRotation(orientation);
        if (this.deviceOrientation == normalized) return;
        this.deviceOrientation = normalized;
        updateMatrices();
    }
    ...
    private void updateMatrices() {
        int recordingRotationDegrees = getRequiredRotation(recordingDeviceOrientation);
        int previewRotationDegrees = getRequiredRotation(deviceOrientation);
        boolean front = isFrontCamera();
        if (previewRotationDegrees != lastLoggedRotation || front != lastLoggedFront || frontVideoMirrorEnabled != lastLoggedFlip) {
            lastLoggedRotation = previewRotationDegrees;
        ...
        Matrix.rotateM(recordingMvpMatrix, 0, recordingRotationDegrees, 0f, 0f, 1f);

        // Preview matrix: handles rotation, mirroring, and aspect ratio
        Matrix.setIdentityM(previewMvpMatrix, 0);
        Matrix.rotateM(previewMvpMatrix, 0, previewRotationDegrees, 0, 0, 1);
        ...
        if (isLandscapeOrientation(userOrientationSetting)
                != isLandscapeOrientation(previewOrientationSetting)) {
            int tmp = orientedVideoWidth;
            orientedVideoWidth = orientedVideoHeight;
            orientedVideoHeight = tmp;
        }
    ...
    private static int normalizeSurfaceRotation(int rotation) {
        switch (rotation) {
            ...
            case Surface.ROTATION_270:
                return rotation;
            default:
                // Configuration.ORIENTATION_* values (1/2) must never be treated
                // as Surface rotations. Unknown values fall back safely to ROTATION_0.
                return Surface.ROTATION_0;
        }
    }

    private static boolean isLandscapeOrientation(@Nullable String orientation) {
        return orientation != null && "landscape".equalsIgnoreCase(orientation);
    }

    private int getDisplayRotationDegrees(int surfaceRotation) {
        switch (normalizeSurfaceRotation(surfaceRotation)) {
            case Surface.ROTATION_90:
                return 90;
            case Surface.ROTATION_180:
                return 180;
    ...
    private int getRequiredRotation(int surfaceRotation) {
        int displayRotationDegrees = getDisplayRotationDegrees(surfaceRotation);
