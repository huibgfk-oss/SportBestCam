...
public class FullscreenPreviewActivity extends AppCompatActivity {
    ...
    private int stableStatusBarHeight = 0;
    private int stableSystemInsetLeft = 0;
    private int stableSystemInsetRight = 0;
    private TextureView textureView;
    private View topBar;
    private View bottomBar;
    private View fsHandballBar;
    ...
    private com.fadcam.handball.FullscreenHandballController handballFullscreenController;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Constants.BROADCAST_ON_RECORDING_PAUSED.equals(action)
                ...
                try {
                    ...
                    if (newPreviewActive && !wasAlreadyActive && textureView != null) {
                        FLog.d("FullscreenPreview", "State callback: preview active — showing TextureView");
                ...
                // If joining an existing preview (was not active before this broadcast),
                // show the preview immediately — no avatar animation needed.
                // Skip if the long-press handler is already running the wake animation.
                if (!wasAlreadyActive && !isPreviewOpenAnimating && textureView != null) {
                    FLog.d("FullscreenPreview", "Joining existing preview — showing TextureView immediately");
                    textureView.setAlpha(1f);
                    if (flFullscreenPreviewAvatar != null) {
                        flFullscreenPreviewAvatar.setVisibility(View.INVISIBLE);
                    }
            ...
            if (handballFullscreenController != null) {
                if (isRecordingActive && !isRecordingPaused) {
                    handballFullscreenController.onVideoRecordingStartedOrResumed();
                } else {
                    handballFullscreenController.onVideoRecordingPausedOrStopped(!isRecordingActive);
                }
            }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        bindViews();
        handballFullscreenController = new com.fadcam.handball.FullscreenHandballController(this);
        gestureScoringEngine = new com.fadcam.handball.GestureScoringEngine(
                textureView,
                () -> FullscreenPreviewActivity.this.hasWindowFocus()
                        && isRecordingActive
                        && !isRecordingPaused,
                (side, delta, confidence) -> {
                    if (handballFullscreenController != null) {
                        handballFullscreenController.onGestureScore(side, delta, confidence);
                    }
                });
        gestureScoringEngine.start();
        if (fsHandballBar != null) fsHandballBar.post(() -> positionHandballBar(controlsVisible));
        enterImmersiveMode();
        setupTextureView();
        setupRecordingTiles();
    ...
    protected void onResume() {
        ...
        if (handballFullscreenController != null) handballFullscreenController.reload();
    ...
    protected void onDestroy() {
        ...
        if (handballFullscreenController != null) { handballFullscreenController.destroy(); handballFullscreenController = null; }
        super.onDestroy();
        autoHideHandler.removeCallbacks(autoHideRunnable);
    ...
    private void bindViews() {
        ...
        fsHandballBar = findViewById(R.id.fs_hb_bar);
    ...
    private void setupTouchHandling() {
        ...
        // Attach to rootLayout AND textureView so touch works when TextureView
        // has a valid Surface bound (which may intercept touches from rootLayout)
    ...
    private void handlePreviewLongPress() {
        ...
        if (isRecordingActive || isRecordingPaused) {
            ...
            if (isPreviewAttachedInRecording) {
                // Toggle ON: wake avatar animation → shrink → iris-open camera reveal
                if (flFullscreenPreviewAvatar != null) {
                ...
                // Toggle OFF: let updatePreviewHintVisibility handle the iris-close
                updatePreviewHintVisibility();
            }
        } else {
    ...
    private void setupSystemInsets() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(rootLayout, (v, insets) -> {
            ...
            // Capture stable status bar height on first non-zero value.
            // When immersive mode hides the status bar, bars.top may become 0 —
            // always use the stable value so the layout doesn't jump.
            if (bars.top > 0) stableStatusBarHeight = bars.top;
            int topInset = stableStatusBarHeight > 0 ? stableStatusBarHeight : bars.top;
            
                        int leftInset = bars.left;
            int rightInset = bars.right;
if (fsHandballBar != null && fsHandballBar.getLayoutParams() instanceof FrameLayout.LayoutParams) {
                FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) fsHandballBar.getLayoutParams();

                // The scoreboard is now the only top rail. Keep it immediately
                // below the current Android safe area and clear of side navigation
                // in both normal and reverse landscape.
                lp.gravity = android.view.Gravity.TOP;
                lp.leftMargin = leftInset + dp(72);
                lp.rightMargin = rightInset + dp(72);
                lp.topMargin = bars.top + dp(4);
                fsHandballBar.setLayoutParams(lp);
            }
            if (bottomBar != null) {
    ...
    private void updatePreviewHintVisibility() {
        ...
        boolean showIdlePlaceholder = !showPreviewSurface && !fullscreenAvatarWakingUp;
    ...
    private void hideControls() {
        ...
        positionHandballBar(false);
    }

    /**
     * SportBestCam: keep the score rail clear of FadCam's own top dock.
     * When FadCam controls are visible the rail slides just below them;
     * when they auto-hide it returns to the top edge.
     */
    private void positionHandballBar(boolean sportBestCamControlsVisible) {
        if (fsHandballBar == null) return;
        fsHandballBar.animate().cancel();
        fsHandballBar.setTranslationY(0f);
    }
    ...
    private void sendSurfaceToService(@Nullable Surface surface, int w, int h) {
        ...
        intent.putExtra("VIDEO_ORIENTATION",
    ...
    private int getCurrentDisplayRotation() {
        try {
            ...
            FLog.w("FullscreenPreview", "Unable to read display rotation", e);
        }
