...
public class RealtimeMediaInvalidationCoordinator {
    ...
    private void startFileObservers() {
        ...
        File currentRoot = new File(externalRoot, Constants.RECORDING_DIRECTORY);
        if (currentRoot.exists()) collectAllDirectories(currentRoot, watchRoots);

        File legacyRoot = new File(externalRoot, Constants.LEGACY_RECORDING_DIRECTORY);
        if (!legacyRoot.equals(currentRoot) && legacyRoot.exists()) {
            collectAllDirectories(legacyRoot, watchRoots);
        }
        if (watchRoots.isEmpty()) return;
     ...
     * (e.g. SportBestCam/SportShot/Back/, with legacy FadCam fallback).
     */
